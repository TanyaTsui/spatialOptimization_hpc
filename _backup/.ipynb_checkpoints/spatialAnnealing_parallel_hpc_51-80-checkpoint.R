...
sa_parallel <- function(nHubs) {
    ...
    write.table(
    resPoints, file='results/resPoints_parallel_51-80.csv', append=TRUE, sep=',', 
    col.names = FALSE
    )
    ...
    write.table(
    resEnergy, file='results/resEnergy_parallel_51-80.csv', append=TRUE, sep=',', 
    col.names = FALSE
    )
...
system.time(test_p <- parSapply(cl, 51:80, sa_parallel))
stopCluster(cl)