...
sa_parallel <- function(nHubs) {
    ...
    write.table(
    resPoints, file='results/resPoints_parallel_2-50.csv', append=TRUE, sep=',', 
    col.names = FALSE
    )
    ...
    write.table(
    resEnergy, file='results/resEnergy_parallel_2-50.csv', append=TRUE, sep=',', 
    col.names = FALSE
    )
...
system.time(test_p <- parSapply(cl, 2:50, sa_parallel))
stopCluster(cl)