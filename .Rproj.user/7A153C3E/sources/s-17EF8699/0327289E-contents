...
def calcTotCostEffectiveness_r(pointsArray, reverse=False, runInR=True): 
    ...
    # make hubs gdf 
    print('make hubNames')
    if isinstance(pointsArray, list): 
        hubNames = [pointsArray[0]] # for where there is only one hub
    else: 
    ...
    print('extract candiHubs using hubNames')
    if reverse:
