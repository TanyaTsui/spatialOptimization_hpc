...
sa_parallel <- function(nHubs) {
    ...
    write.table(
    resPoints, file='results/resPoints_230331_2-3.csv', append=TRUE, sep=',', 
    col.names = FALSE
    )
    ...
    write.table(
    resEnergy, file='results/resEnergy_230331_2-3.csv', append=TRUE, sep=',', 
    col.names = FALSE
    )
...
# nCores <- detectCores()
cl <- makeCluster(2)
system.time(test_p <- parSapply(cl, 2:3, sa_parallel))
stopCluster(cl)